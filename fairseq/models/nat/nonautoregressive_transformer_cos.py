...
def del_start_end(cos_sim):
    ...
    for i in range(length):
        ...
        for j in range(len(cos_sim[i])):
            if j == 0 or j == len(cos_sim[i]) - 1 or j == num:
                continue
            temp.append(cos_sim[i][j])
        num += 1
        res.append(temp)
    return res
...
@register_model("nonautoregressive_transformer_cmlmc")
...
class NATransformerDecoder(FairseqNATDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        # look neighbor
        self.insertNeighborAttn = args.insertNeighborAttn
        self.concatLast = args.concatLast
        if self.concatLast:
            self.reduce_concat1 = torch.nn.Linear(self.args.decoder_embed_dim * 2, self.args.decoder_ffn_embed_dim, bias=False)
            self.reduce_concat2 = torch.nn.Linear(self.args.decoder_ffn_embed_dim, self.args.decoder_embed_dim, bias=False)
            self.concat_activation_fn = utils.get_activation_fn(
                activation="gelu"
            )
            self.concat_dropout = torch.nn.Dropout(0.3)
    ...
    def forward(self, normalize, encoder_out, prev_output_tokens, step=0, **unused):
        ...
        return [F.log_softmax(decoder_out, -1), features.clone()] if normalize else [decoder_out, features.clone()]
    ...
    def build_neighbor_mask(self, tensor, decoder_padding_mask):
        dim = tensor.size(0)
        triu_mat = torch.triu(torch.ones([dim, dim]), 2).bool()
        tril_mat = torch.tril(torch.ones([dim, dim]), -2).bool()
        self._future_mask = (triu_mat | tril_mat).float()
        self._future_mask.masked_fill_(self._future_mask.eq(1), float('-inf'))
        self._future_mask = self._future_mask.unsqueeze(0).repeat(decoder_padding_mask.size(0), 1, 1)
        self._future_mask = self._future_mask.to(tensor.device)
        pad_mask = decoder_padding_mask.unsqueeze(-1).repeat(1, 1, dim)
        self._future_mask.masked_fill_(pad_mask, float('0.'))
        # expand to head_num * batch, tgt_len, tgt_len
        head_num = self.layers[0].self_attn.num_heads
        self._future_mask = self._future_mask.repeat(1, head_num, 1).view(decoder_padding_mask.size(0) * head_num, dim, dim)
    ...
    def extract_features(
        ...
        **unused
        ...
        if self.insertNeighborAttn:
            self_neighbor_mask = self.build_neighbor_mask(x, decoder_padding_mask)
            for i, layer in enumerate(self.layers):
                ...
                x, attn, _ = layer(
                    ...
                    self_neighbor_mask=self_neighbor_mask,
        ...
        if self.concatLast:
            ...
            x = x.transpose(0, 1)
            length = x.size()[1]
            x_left_right = torch.cat([x[:, :length-2], x[:, 2:]], dim=-1)
            concat_to_ffn = self.concat_activation_fn(self.reduce_concat1(x_left_right))
            x[:, 1:length-1] += self.reduce_concat2(concat_to_ffn)
            x[:, 1:length-1] = self.concat_dropout(self.concat_activation_fn(x[:, 1:length-1]))
            x = x.transpose(0, 1)
...
@register_model_architecture(
    "nonautoregressive_transformer_cmlmc", "nonautoregressive_transformer_cmlmc"
)
...
@register_model_architecture(
    "nonautoregressive_transformer_cmlmc", "nonautoregressive_transformer_cmlmc_wmt_en_de"
)
